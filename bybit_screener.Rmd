...
if(funding == TRUE) {
  ...
  while(current.ticker <= tickercount) {
    ...
    fundingrate <- suppressWarnings(as.numeric(as.character(unlist(fundingrate[]))))
    
    perpdata[current.ticker, 'Funding'] <- fundingrate[6] * 109500
    current.ticker <- current.ticker + 1
  }
...
if(longshort_ratio == TRUE) {
  ...
  while(current.ticker <= tickercount) {
    ...
    perpdata[current.ticker, 'LSratio'] <- (ratio[1, 2] - ratio[1, 3]) * 100
    current.ticker <- current.ticker + 1
  }
